...
object AllMain1 extends Constants{

  def main(args: Array[String]) {
    taskmain44()
    //yunketangMain.taskmain3()
      ...
      .filter(x=>{
       ...
       .map(x => {
         try{
           ...
           if(sql_type=="u") obj.put("id",json.getJSONObject("o2").getString("_id"))
           ...
           for( a <- 0 to array.size()-1) {
             ...
             if (table == "xh_cloudwork_parts.work"&&sql_type=="i") {
               ...
               if (data.getString("Name")=="topic_num"){
                 val topic_num=data.getString("Value")
                 obj.put("topic_num",topic_num)
               }
               if (data.getString("Name")=="upto_time"){
                 val upto_time=data.getString("Value")
                 obj.put("upto_time",upto_time)
               }
             ...
             if (table == "xh_cloudwork_parts.work"&&sql_type=="u") {
...
//             }
      ...
      .apply(new ByWindow33())
      .addSink(new  MySqlSink244())
...
class ByWindow33() extends AllWindowFunction[JSONObject, Iterable[JSONObject], TimeWindow]{
...
class MySqlSink244() extends RichSinkFunction[Iterable[JSONObject]] with Constants {
  // 定义sql连接、预编译器
    ...
    try{
      for(value <-values) {
        ...
        if (table == "xh_cloudwork_parts.work"&&sql_type=="i") {
          ...
          inserWorkID.setInt(6,value.getInteger("topic_num"))
          inserWorkID.setInt(7,value.getInteger("upto_time"))
        ...
        if ((table == "xh_cloudwork_parts.studentWork" || table=="xh_cloudwork_exam.studentExam")&&sql_type=="u") {
        ...
        if(table.contains("ClassroomBase")){
          ...
          insertYunkeTang.setInt(20,school_id)
